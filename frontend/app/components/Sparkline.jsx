...
export default function Sparkline({ points, width = 280, height = 60 }) {
  ...
  return (
    <div className="flex items-center gap-4">
      <svg viewBox={`0 0 ${width} ${height}`} width="100%" height={height} preserveAspectRatio="none" aria-hidden="true" className="flex-1 min-w-0">
        <path d={`${d} L${width},${height} L0,${height} Z`} fill={color} opacity="0.09" />
        <path d={d} fill="none" stroke={color} strokeWidth="2" strokeLinejoin="round" />
      </svg>
      <span className="text-[15px] font-bold tnum whitespace-nowrap" style={{ color }}>
        {up ? "▲" : "▼"} {pct.toFixed(2)}%
      </span>
    </div>
  );
